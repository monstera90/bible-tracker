...
window.initMdEditorModule = function(deps){
  ...
  // переносится на обёрнутый текст (это внутреннее состояние CM,
  // state.selection, живёт и без DOM-фокуса).
  // БЕЗ cmView.focus() (убран 14.09, ТЗ пользователя): раньше эта строка
  // возвращала DOM-фокус редактору, чтобы можно было сразу продолжить
  // печатать — но на телефоне DOM-фокус на CodeMirror автоматически
  // поднимает системную клавиатуру, а preventDefault на mousedown/click
  // кнопок попапа (см. bindFormatBtn) её не останавливает, потому что
  // фокус здесь ставился программно, уже ПОСЛЕ этих событий. Из-за этого
  // при каждом выборе стиля в попапе "Ж/К/П/Ч" клавиатура выезжала заново
  // и перекрывала сам попап. dispatch() выше обновляет и текст, и рендер
  // CM независимо от фокуса — визуально обёртка применяется как и раньше,
  // просто без принудительного фокуса и без непрошеной клавиатуры.
